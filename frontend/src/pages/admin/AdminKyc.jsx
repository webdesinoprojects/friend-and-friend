...
export default function AdminKyc() {
  ...
  return <AdminShell eyebrow="Trust operations" title="KYC Approvals" text="Review identity evidence, registration details and approval history.">
    <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
      <div className="flex gap-2">{["ALL", "PENDING", "VERIFIED", "REJECTED"].map(value => <button key={value} onClick={() => setFilter(value)} className={`rounded-full px-4 py-2 text-xs font-black ${filter === value ? "bg-black text-white" : "border border-black/10 bg-white"}`}>{value}</button>)}</div>
      <label className="flex min-w-[280px] items-center gap-3 rounded-2xl border border-black/10 bg-white px-4 py-3"><Search size={18}/><input value={query} onChange={e => setQuery(e.target.value)} placeholder="Search name, phone, email or ID" className="w-full bg-transparent text-sm font-bold outline-none"/></label>
    </div>
...
function ReviewDialog({ item, reason, setReason, close, decide }) {
  ...
  return <div className="fixed inset-0 z-[10000] overflow-y-auto bg-black/65 p-3 sm:p-6"><div className="mx-auto my-3 max-w-6xl overflow-hidden rounded-[2rem] bg-[#fffdf9] shadow-2xl">
    <header className="sticky top-0 z-10 flex items-center justify-between border-b border-black/10 bg-white/95 p-5 backdrop-blur"><div className="flex items-center gap-4"><Avatar row={item} large/><div><p className="text-xs font-black uppercase tracking-[.16em] text-[#df7a31]">Identity application</p><h2 className="text-2xl font-black sm:text-3xl">{item.fullName}</h2><p className="text-sm font-bold text-black/45">{item.role} · Registered {dateTime(item.createdAt)}</p></div></div><button onClick={close} className="grid h-11 w-11 place-items-center rounded-full bg-black text-white" aria-label="Close"><X/></button></header>
    <div className="grid gap-6 p-5 lg:grid-cols-[1fr_1.15fr]">
      ...
      <div className="space-y-5"><div className="grid gap-4 sm:grid-cols-2"><Document title="Live selfie" src={item.referenceSelfie}/><Document title="Profile picture" src={item.profileImage}/></div>
        ...
        <div className="grid grid-cols-2 gap-3"><button onClick={()=>decide("REJECTED")} className="inline-flex items-center justify-center gap-2 rounded-full bg-rose-600 px-5 py-3.5 font-black text-white"><XCircle/>Reject</button><button onClick={()=>decide("VERIFIED")} className="inline-flex items-center justify-center gap-2 rounded-full bg-emerald-600 px-5 py-3.5 font-black text-white"><CheckCircle2/>Approve</button></div>
      </div>
    </div>
  </div></div>;
}
...
function Section({ title, icon:Icon, children }) { return <section className="rounded-3xl border border-black/10 bg-[#f8f6f2] p-5"><h3 className="mb-4 flex items-center gap-2 font-black"><Icon size={19}/>{title}</h3>{children}</section> }
