                ...
                {bookings.map((booking) => {
                  ...
                  return (
                    <article key={booking.id} className="min-w-0 self-start border border-slate-200 bg-white p-4 shadow-[0_10px_32px_rgba(15,23,42,0.07)] transition hover:border-slate-300">
                      ...
                      <div className="mt-4 grid grid-cols-2 gap-3 border-y border-slate-100 bg-slate-50 p-3 text-xs">
                      ...
                      <div className="mt-4 flex flex-wrap items-center justify-between gap-3">
                        ...
                        <div className="flex gap-2">
                          {String(booking.paymentStatus || "").toUpperCase() === "PAID" ? <a href={`/app/provider/chat?booking=${booking.id}`} className="inline-flex items-center gap-2 bg-[#fff0df] px-3 py-2 text-xs font-black text-[#a65d28]"><MessageCircle size={15}/>Chat</a> : null}
                          {status === "PENDING" ? <button type="button" onClick={async () => { try { const saved = await acceptBookingApi(booking.id); setBookings((rows) => rows.map((item) => item.id === booking.id ? { ...item, ...saved } : item)); notify("Booking accepted. Payment is now open for the user.", "success"); } catch (error) { notify(error?.response?.data?.message || "Could not accept booking.", "error"); } }} className="bg-emerald-600 px-4 py-2.5 text-xs font-black text-white">Accept</button> : null}
                        </div>
                      </div>
