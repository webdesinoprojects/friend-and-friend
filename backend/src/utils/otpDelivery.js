...
function sendEmailOtp(email, otp) {
  ...
  if (!apiKey) {
    if (process.env.NODE_ENV === "production") throw new Error("Resend is not configured.");
    console.info(`DEV_EMAIL_OTP: ${email} -> ${otp}`);
    return { development: true };
  }
  return sendTransactionalEmail({
    to: email,
    subject: `${otp} is your BuddyBOOK verification code`,
    ...
    text: `Your BuddyBOOK email verification code is ${otp}. It expires in 10 minutes. Do not share it.`,
    tag: "email_otp",
  });
}
