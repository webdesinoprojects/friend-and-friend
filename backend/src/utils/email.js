...
function template({ name, eyebrow, title, intro, status, statusColor, reason, ctaLabel, ctaUrl, approved }) {
  ...
  <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:650px;background:#ffffff;border:1px solid #edd8c7;border-radius:30px;overflow:hidden;box-shadow:0 18px 55px rgba(52,35,23,.10)">
    <tr><td style="padding:32px 38px 20px;background:linear-gradient(135deg,#fffdf9,#fff3e7)"><div style="font-size:25px;font-weight:900;color:#10203e">Buddy<span style="color:#f5b800">BOOK</span></div><div style="margin-top:8px;color:#718096;font-size:12px;font-weight:700">Safe Meetups. Real Connections.</div></td></tr>
    <tr><td style="padding:18px 38px 38px">
      ...
      <p style="font-size:13px;line-height:1.6;color:#718096;margin:0">For your safety, BuddyBOOK will never ask you to share an OTP or password by email. If you did not submit this application, contact support.</p>
    </td></tr>
  </table><p style="font-size:11px;color:#9aa2af;margin:18px 0">© ${new Date().getFullYear()} BuddyBOOK · Automated verification notification</p>
...
function sendApplicationReceived(application) {
  ...
  return sendTransactionalEmail({
    to: application.email, subject: "We received your BuddyBOOK application",
    ...
    text: `Hi ${application.fullName}, your BuddyBOOK application is under admin review. Your account will be created only after approval.`, tag: "application_received",
  });
}
...
function sendApplicationDecision(application, status, reason) {
  ...
  return sendTransactionalEmail({
    ...
    subject: approved ? "Your BuddyBOOK profile has been approved" : "Your BuddyBOOK application was not approved",
    html: template({
      name: application.fullName,
      eyebrow: approved ? "Verification complete" : "Application decision",
      title: approved ? "Welcome to BuddyBOOK" : "Your application needs attention",
      intro: approved ? "Your identity and profile were approved by our admin team. Your BuddyBOOK account has now been created and you can log in using the same email/phone and password." : "Our admin team could not approve the submitted profile. No BuddyBOOK account has been created, so these credentials cannot be used to log in.",
      status: approved ? "Approved · Account created" : "Rejected · Account not created",
      statusColor: approved ? "#198754" : "#dc3545", reason,
      ctaLabel: approved ? "Log in to BuddyBOOK" : "Review and update application",
      ctaUrl: approved ? `${base}/login` : `${base}/application-review`, approved,
    }),
    text: approved ? `Hi ${application.fullName}, your BuddyBOOK application was approved. Reason: ${reason || "Verification completed successfully"}. You can now log in.` : `Hi ${application.fullName}, your BuddyBOOK application was rejected. Reason: ${reason}. No account was created.`,
